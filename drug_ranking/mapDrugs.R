...
mapDrugs <- function(target_module_info){
  ...
  Drugs_DrugBank <- read.table(file = "Drugs_DrugBank.txt", header = T, sep = '\t', check.names = F, quote = "")
  DTI_DrugBank <- read.table(file = "DTI_DrugBank.txt", header = T, sep = '\t', check.names = F, quote = "")
  Drugs_TTD <- fread("Drugs_TTD.txt", skip = 28, header = F)
  Drugs_supp_TTD <- fread("Drugs_supp_TTD.txt", skip = 27, header = F)
  DTI_TTD <- read.table(file = "DTI_TTD.txt", header = T, sep = '\t', check.names = F, quote = "")
